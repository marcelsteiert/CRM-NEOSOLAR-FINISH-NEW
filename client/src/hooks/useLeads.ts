...
export function useUpdateRoleDefaults() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (data: { defaults: Record<string, string[]>; applyToUsers?: boolean }) =>
      api.put<{ data: Record<UserRole, string[]>; updatedUsers: number }>('/users/role-defaults', data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['roleDefaults'] })
