...
import {
  ...
  FolderOpen,
  FileSignature, Receipt, Award, GraduationCap, Shield, HeartPulse, Wallet, HelpCircle,
} from 'lucide-react'
import {
...
interface PersonnelDocumentSectionProps {
  personnelId: string
}

// ── Personal-Ordnerstruktur ──
const FOLDERS: { id: string; label: string; color: string; icon: typeof FileText }[] = [
  { id: 'Arbeitsvertrag',       label: 'Arbeitsvertrag',       color: '#F59E0B', icon: FileSignature },
  { id: 'Lohnabrechnungen',     label: 'Lohnabrechnungen',     color: '#34D399', icon: Receipt },
  { id: 'Zeugnisse',            label: 'Zeugnisse',            color: '#A78BFA', icon: Award },
  { id: 'Diplome',              label: 'Diplome',              color: '#22D3EE', icon: GraduationCap },
  { id: 'AHV / Versicherung',   label: 'AHV / Versicherung',   color: '#60A5FA', icon: Shield },
  { id: 'Krankheit / Unfall',   label: 'Krankheit / Unfall',   color: '#F87171', icon: HeartPulse },
  { id: 'Spesen',               label: 'Spesen',               color: '#FBBF24', icon: Wallet },
...
export default function PersonnelDocumentSection({ personnelId }: PersonnelDocumentSectionProps) {
  const { user } = useAuth()
  const qc = useQueryClient()
  const { data: docsRes } = usePersonnelDocuments(personnelId)
  ...
  const uploadFiles = useCallback(async (files: FileList | File[], folderPath: string) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const storagePath = `personnel/${personnelId}/${safeFolder}/${timestamp}_${safeName}`
  ...
  }, [personnelId, user?.id, uploading, qc])
        ...
        <div className="flex-1 min-w-0">
          {uploading ? (
            ...
            <>
              ...
              <p className="text-[10px] text-text-dim">Drag & Drop in einen Ordner unten oder klicken (→ Arbeitsvertrag)</p>
            </>
          )}
        </div>
      </div>
