...
export default function Sidebar() {
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-[72px] flex flex-col items-center py-5 z-50 border-r border-border bg-bg/80 backdrop-blur-2xl">
      {/* Logo */}
      <div className="relative group/logo mb-6 shrink-0">
        <div
      ...
      <nav className="flex-1 flex flex-col items-center gap-0 py-1 overflow-y-auto" aria-label="Hauptnavigation">
        {navGroups.map((group, gi) => (
          <div key={gi} className="w-full flex flex-col items-center">
            {/* Separator between groups */}
            {gi > 0 && (
              <div className="w-8 h-px my-2" style={{ background: 'rgba(255,255,255,0.06)' }} />
            )}
