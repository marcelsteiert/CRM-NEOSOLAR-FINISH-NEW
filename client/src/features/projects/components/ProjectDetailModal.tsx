            ...
            <div className="w-full md:w-[280px] shrink-0 space-y-5">
              ...
              <div className="glass-card p-4">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[10px] font-bold tracking-[0.08em] uppercase text-text-dim">Notizen</span>
                </div>
                {isEditing ? (
                  <textarea value={editNotes} onChange={(e) => setEditNotes(e.target.value)} placeholder="Notizen zum Projekt..." rows={4} className="glass-input w-full px-3 py-2 text-[12px] resize-none" />
                ) : (
                  <p className="text-[12px] text-text-sec whitespace-pre-wrap">{project.notes || 'Keine Notizen'}</p>
                )}
              </div>
              ...
              {/* Aktivitäten */}
              <div className="glass-card p-4">
                <h4 className="text-[10px] font-bold tracking-[0.08em] uppercase text-text-dim mb-3">
                  Aktivitäten ({sortedActivities.length})
                </h4>
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                  {sortedActivities.length === 0 ? (
                    <p className="text-[10px] text-text-dim text-center py-3">Noch keine Aktivitäten</p>
                  ) : (
                    sortedActivities.map((act) => {
                      ...
                      return (
                        <div key={act.id} className="flex gap-2.5">
                          <div className="w-6 h-6 rounded-[7px] flex items-center justify-center shrink-0 mt-0.5" style={{ background: `color-mix(in srgb, ${color} 12%, transparent)` }}>
                            <IconComp size={11} strokeWidth={1.8} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-[11px] text-text-sec leading-relaxed">{act.text}</p>
                            <div className="flex items-center gap-2 mt-0.5 text-[9px] text-text-dim">
