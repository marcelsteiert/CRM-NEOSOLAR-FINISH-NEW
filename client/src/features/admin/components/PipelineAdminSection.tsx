...
import { Plus, ChevronUp, ChevronDown, Pencil, Check, X, Trash2, AlertTriangle } from 'lucide-react'
import {
  usePipelines,
  useCreatePipeline,
  useUpdatePipeline,
...
export default function PipelineAdminSection() {
  ...
  const [confirmDeleteBucket, setConfirmDeleteBucket] = useState<{ pipelineId: string; bucketId: string } | null>(null)
  ...
  const handleDeleteBucket = (pipelineId: string, bucketId: string) => {
    deleteBucket.mutate({ pipelineId, bucketId })
  ...
  return (
    <div className="space-y-4">
      ...
      {pipelines.map((pipeline) => {
        ...
        const isConfirmingDelete = confirmDeletePipeline === pipeline.id
        return (
          <div key={pipeline.id} className="glass-card p-4">
            {/* Pipeline Header */}
            <div className="flex items-center gap-2 mb-3">
              {editingPipelineId === pipeline.id ? (
                ...
                <>
                  ...
                  <span className="text-[10px] text-text-dim mr-2">{sortedBuckets.length} Stufen</span>
                  <button
                    type="button"
                  ...
                  {isConfirmingDelete ? (
                    <div className="flex items-center gap-1.5 ml-1">
                      ...
                      <span className="text-[10px] text-red-400 font-semibold">Löschen?</span>
                      <button
                        type="button"
            ...
            <div className="space-y-1.5 ml-2">
              {sortedBuckets.map((bucket, idx) => {
                const isConfirmBucketDelete = confirmDeleteBucket?.bucketId === bucket.id
                return (
                  <div
                    key={bucket.id}
                    ...
                    {editingBucketId === bucket.id ? (
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          {isConfirmBucketDelete ? (
                            <div className="flex items-center gap-0.5 ml-1">
                              <button
                                type="button"
                                onClick={() => handleDeleteBucket(pipeline.id, bucket.id)}
                                className="text-red-400 hover:text-red-300 p-0.5"
                              >
                            ...
                            <button
                              type="button"
                              onClick={() => setConfirmDeleteBucket({ pipelineId: pipeline.id, bucketId: bucket.id })}
                              className="text-text-dim hover:text-red-400 p-0.5 transition-colors"
                              title="Stufe löschen"
                            >
